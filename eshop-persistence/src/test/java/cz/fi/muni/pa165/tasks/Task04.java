...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductByName(){
		List<Product> found = em.createQuery("select p from Product p where p.name = :name",Product.class)
								.setParameter("name", "Flashlight").getResultList();
	...
	public void groupByAndOrderBy(){
		List<Object[]> found= em.createQuery("select p.color,count(p) from Product p group by p.color order by p.color",
	...
	public void groupByAndOrderBySelectNew() {
		List<ColorCount> colorCounts = em.createQuery("select new cz.fi.muni.pa165.tasks.ColorCount(p.color,count(p)) " +
													"from Product p group by p.color order by p.color",ColorCount.class)
													.getResultList();
	...
	public void findProductsAddedYesterday() {
		...
		List<Product> products= em.createQuery("select p from Product p where p.addedDate = :date",Product.class)
									.setParameter("date", yesterday).getResultList();
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em.createQuery("select new cz.fi.muni.pa165.tasks.ProductAndCategory(p,c) " +
													"from Product p left join p.categories c on c.name= 'Kitchen' order by p.name,c.name"
